...
>
> **NEXT SESSION FOCUS: tablet / mobile UX pass.** Read `MOBILE_TABLET_HANDOFF.md` (root) first — it has the target device (Acer Iconia Tab A16), the list of pages and components to audit, the likely pain points, and the test checklist. The customer is using the app primarily on that tablet; some surfaces (especially the new `/admin/commander` page) were laid out desktop-first and need a sweep.
>
