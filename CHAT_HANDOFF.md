...
> **2026-05-01 (later same day) — local dev is now PostgreSQL too.** SQLite retired. New `docker-compose.yml` at repo root brings up a Postgres 16 container; `appsettings.json`'s `ConnectionStrings:DefaultConnection` defaults to it; `Program.cs` is now Postgres-only (all five SQLite self-heal blocks deleted, ~136 lines lighter). Migration Safety Rule 3 collapsed from "SQLite + Postgres self-heal" to just Postgres. **First-time setup on a fresh checkout:** `docker compose up -d`, copy `appsettings.Local.example.json` → `Local.json`, `cd API && dotnet ef database update`, `dotnet run`. **If your existing checkout has `API/dochadzkovnik.db` tracked in git**, run `git rm --cached API/dochadzkovnik.db` once (the file is now gitignored). Full runbook in `SECRETS.md` § Local development.
>
> **NEXT SESSION FOCUS: warehouse + financial tracking.** Read `WAREHOUSE_PLAN.md` (root) first. The customer wants a "warehouse" concept tied to locations, the ability to plan how much material a job will need and log purchases, and longer term a per-location P&L view (revenue vs. material + labour spend). The plan recommends starting with a lightweight Plans + Purchases model (no live stock counts in V1), shipped behind a `WarehousePhase1` feature flag, same pattern Commander used. **Do not start before getting answers to the 8 open questions in `WAREHOUSE_PLAN.md` § "Open questions for the customer"** — questions 1–5 are blocking.
>
> Latest hotfix on top of V1.3.1: **V1.3.2 (2026-04-30) — removed worker phone numbers from the public kiosk view.** The "Bez záznamu hodín…" list was rendering phone numbers as clickable `tel:` links on a wall-mounted tablet. The underlying anonymous endpoint `/api/kiosk/missing-hours-overview` was also serving the numbers as plain JSON. Fixed end-to-end (backend DTO, controller, frontend interface, template). Managers still see numbers via the JWT-protected admin Employees pages.
>
> Previous session (2026-05-01) shipped a lot. Skim then move on:
> 1. **Tablet / mobile UX pass**, including a new `.touch-target` utility, dvh-based heights everywhere, stacked-card layouts below `md` for `/admin/commander` (both Detail and Prehľad), `/admin/materials`, and the kiosk `Šichtovnica` weekly grid. Full inventory in `MOBILE_TABLET_HANDOFF.md` § "What was done this session".
> 2. **Materials prod fixes.** `MaterialsController.Create` now resurrects soft-deleted name matches instead of 409'ing. Startup seed reactivates the 10 predefined materials on every deploy. New unified PostgreSQL self-heal block converts any text-typed `CreatedAt`/`UpdatedAt`/`PricePerUnit`/etc. to their proper `timestamp` / `numeric(p,s)` types in one go (the `DROP DEFAULT, ALTER TYPE, SET DEFAULT 0` pattern was needed because the legacy text columns had `DEFAULT '0'`). EF Core SQL logging dropped to Warning so self-heal SQL no longer floods Railway and trips the 500 logs/sec rate limit.
> 3. **iOS status-bar white glow.** `ThemeService.applyTheme()` now writes an unscoped `<meta name="theme-color">` so the iOS status bar follows the app's own dark-mode toggle, not just the OS preference. Static media-scoped tags in `index.html` updated to slate-50 / slate-900 (page wrapper colours, not navbar). `html` and `html.dark body` backgrounds pinned in `styles.css` so any overscroll never reveals user-agent white.
>
> **Previous session (2026-05-01) shipped: Commander integration M1 + M2.** Read-only fleet view, ride aggregates ("Prehľad jázd"), per-ride start/stop on a Leaflet map, live-position polling, all behind the `CommanderIntegration` feature flag. See `COMMANDER_PLAN.md` for the design and the new session log entry in `PROJECT_NOTES.md` for the file inventory. Local Commander credentials are in `API/appsettings.Local.json`; Railway dev has `Commander__BaseUrl` / `__Username` / `__Password` set. The Commander login is an **API user** ("Dochadzkovnik-API"), not the customer's web portal login — set up via Commander → Nastavenia → Prepoje na tretiu stranu.
>
...
- **The app is a PWA.** The customer's primary device is an **Android tablet (Acer Iconia Tab A16, 16" IPS 1920×1200)** used as an installed PWA in landscape on a stand. Every UI change must be tested on Android Chrome standalone first (Add to Home Screen), and on iPhone Safari standalone as a secondary check. See the `📱 PWA / Mobile Compatibility` section of `PROJECT_NOTES.md` for the full checklist (Android-specific gotchas section included).
