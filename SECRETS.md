...
| `Google__DocumentAi__CredentialsPath` | `Google:DocumentAi:CredentialsPath` | **Preferred for local dev.** Absolute path to the service-account JSON key file on disk. The API reads the file at startup. Keeps `appsettings.Local.json` clean and avoids escaping multi-line JSON inside a JSON string. Example: `C:/Users/Michael/secrets/dochadzkovnik-document-ai.json`. **The JSON file must not live inside the repo.** Pick a folder outside the working tree (e.g. `C:/Users/Michael/secrets/`). |
| `Google__DocumentAi__CredentialsJson` | `Google:DocumentAi:CredentialsJson` | Google Cloud service-account JSON (full contents). **Used in Railway prod where the env var UI accepts multi-line values.** Either this OR `CredentialsPath` must be set when InvoiceScanning is on. **Treat as a credential — never logged, never echoed.** When neither is set the API skips registering the Document AI client and the InvoicesController returns a Slovak "OCR nie je nakonfigurované" error instead of crashing. |
| `Google__DocumentAi__ProjectId` | `Google:DocumentAi:ProjectId` | Google Cloud project ID hosting the Document AI processor | E.g. `dochadzkovnik-456789`. Visible in the GCP console under the project dropdown. |
| `Google__DocumentAi__Location` | `Google:DocumentAi:Location` | Document AI region | **Use `eu` for SK invoices** (data residency + the regional endpoint `eu-documentai.googleapis.com` is auto-selected by the client). |
| `Google__DocumentAi__ProcessorId` | `Google:DocumentAi:ProcessorId` | The 16-char Invoice Parser processor ID | Copy from the processor detail page in the GCP console. The client builds the full resource name `projects/{ProjectId}/locations/{Location}/processors/{ProcessorId}` automatically. |
